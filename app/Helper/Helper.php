...
class Helper
{
    ...
    {
        if ($branch == 0):
            $stock = DB::select("SELECT tbl7.*, ((tbl7.balance_qty + tbl7.sreturn)-tbl7.preturn) AS balanceQty FROM (SELECT tbl6.*, IFNULL(SUM(CASE WHEN prd.batch_number = tbl6.batch_number AND prd.product_id = tbl6.pid AND pret.stock_updated_at IS NULL AND pret.deleted_at IS NULL THEN prd.rqty END), 0) AS preturn FROM(SELECT tbl5.*, IFNULL(SUM(CASE WHEN srd.batch_number = tbl5.batch_number AND srd.product_id = tbl5.pid AND sr.branch_id = ? AND sr.stock_updated_at IS NULL AND sr.deleted_at IS NULL THEN srd.rqty END), 0) AS sreturn, (tbl5.medicine + tbl5.pom + tbl5.ph_direct) AS billed, (tbl5.transfer_in + tbl5.purchased_qty) AS tot_in, ((tbl5.transfer_in + tbl5.purchased_qty) - (tbl5.medicine + tbl5.pom + tbl5.ph_direct + tbl5.transfer_out)) AS balance_qty FROM (SELECT tbl4.*, IFNULL(SUM(CASE WHEN med.batch_number = tbl4.batch_number AND med.medicine = tbl4.pid AND med.branch_id = ? AND med.status = 1 AND med.stock_updated_at IS NULL AND med.deleted_at IS NULL THEN med.qty END), 0) AS medicine FROM (SELECT tbl3.*, IFNULL(SUM(CASE WHEN pomd.batch_number = tbl3.batch_number AND pomd.product = tbl3.pid AND pom.branch = ? AND pom.stock_updated_at IS NULL AND pom.deleted_at IS NULL THEN pomd.qty END), 0) AS pom FROM (SELECT tbl2.*, IFNULL(SUM(CASE WHEN pr.batch_number = tbl2.batch_number AND pr.product = tbl2.pid AND ph.branch = ? AND ph.stock_updated_at IS NULL AND ph.deleted_at IS NULL THEN pr.qty END), 0) AS ph_direct FROM (SELECT tbl1.*, IFNULL(SUM(CASE WHEN td.batch_number = tbl1.batch_number AND td.product = tbl1.pid AND t.approved = 1 AND t.to_branch = ? AND t.stock_updated_at IS NULL AND t.deleted_at IS NULL THEN td.qty END), 0) AS transfer_in, IFNULL(SUM(CASE WHEN td.batch_number = tbl1.batch_number AND td.product = tbl1.pid AND t.approved = 1 AND t.from_branch = ? AND t.stock_updated_at IS NULL AND t.deleted_at IS NULL THEN td.qty END), 0) AS transfer_out FROM (SELECT p.id AS pid, p.product_name AS pname, pd.batch_number, pd.expiry_date, SUM(pd.qty) AS purchased_qty FROM products p LEFT JOIN purchase_details pd ON p.id = pd.product LEFT JOIN purchases pur ON pur.id = pd.purchase_id WHERE IF(? > 0, p.id = ?, 1) AND IF(? > 0, pur.branch_id = ?, 1) AND pur.deleted_at IS NULL AND pur.stock_updated_at IS NULL GROUP BY p.id, p.product_name, pd.batch_number, pd.expiry_date) AS tbl1 LEFT JOIN product_transfer_details td ON tbl1.pid = td.product LEFT JOIN product_transfers t ON t.id = td.transfer_id GROUP BY tbl1.pid, tbl1.batch_number, tbl1.expiry_date) AS tbl2 LEFT JOIN pharmacy_records pr ON tbl2.pid = pr.product LEFT JOIN pharmacies ph ON ph.id = pr.pharmacy_id GROUP BY tbl2.pid, tbl2.batch_number, tbl2.expiry_date) AS tbl3 LEFT JOIN post_operative_medicine_details pomd ON tbl3.pid = pomd.product LEFT JOIN post_operative_medicines pom ON pom.id = pomd.pom_id GROUP BY tbl3.pid, tbl3.batch_number, tbl3.expiry_date) AS tbl4 LEFT JOIN patient_medicine_records med ON tbl4.pid = med.medicine GROUP BY tbl4.pid, tbl4.batch_number, tbl4.expiry_date) AS tbl5 LEFT JOIN sales_return_details srd ON tbl5.pid = srd.product_id LEFT JOIN sales_returns sr ON sr.id = srd.return_id GROUP BY tbl5.pid, tbl5.batch_number, tbl5.expiry_date) AS tbl6 LEFT JOIN purchase_return_details prd ON prd.product_id = tbl6.pid LEFT JOIN purchase_returns pret ON pret.id = prd.return_id GROUP BY tbl6.pid, tbl6.batch_number, tbl6.expiry_date) AS tbl7", [$branch, $branch, $branch, $branch, $branch, $branch, $product, $product, $branch, $branch]);
        else:
            $stock = DB::select("SELECT tbl7.*, ((tbl7.balance_qty + tbl7.sreturn)-tbl7.preturn) AS balanceQty FROM (SELECT tbl6.*, IFNULL(SUM(CASE WHEN prd.batch_number = tbl6.batch_number AND prd.product_id = tbl6.pid AND pret.stock_updated_at IS NULL AND pret.deleted_at IS NULL THEN prd.rqty END), 0) AS preturn FROM(SELECT tbl5.*, IFNULL(SUM(CASE WHEN srd.batch_number = tbl5.batch_number AND srd.product_id = tbl5.pid AND sr.branch_id = ? AND sr.stock_updated_at IS NULL AND sr.deleted_at IS NULL THEN srd.rqty END), 0) AS sreturn, (tbl5.medicine + tbl5.pom + tbl5.ph_direct) AS billed, (tbl5.transfer_in + tbl5.purchased_qty) AS tot_in, ((tbl5.transfer_in + tbl5.purchased_qty) - (tbl5.medicine + tbl5.pom + tbl5.ph_direct + tbl5.transfer_out)) AS balance_qty FROM (SELECT tbl4.*, IFNULL(SUM(CASE WHEN med.batch_number = tbl4.batch_number AND med.medicine = tbl4.pid AND med.branch_id = ? AND med.status = 1 AND med.stock_updated_at IS NULL AND med.deleted_at IS NULL THEN med.qty END), 0) AS medicine FROM (SELECT tbl3.*, IFNULL(SUM(CASE WHEN pomd.batch_number = tbl3.batch_number AND pomd.product = tbl3.pid AND pom.branch = ? AND pom.stock_updated_at IS NULL AND pom.deleted_at IS NULL THEN pomd.qty END), 0) AS pom FROM (SELECT tbl2.*, IFNULL(SUM(CASE WHEN pr.batch_number = tbl2.batch_number AND pr.product = tbl2.pid AND ph.branch = ? AND ph.stock_updated_at IS NULL AND ph.deleted_at IS NULL THEN pr.qty END), 0) AS ph_direct FROM (SELECT tbl1.pid, tbl1.pname, tbl1.expiry_date, tbl1.purchased_qty, td.batch_number, IFNULL(SUM(CASE WHEN td.product = tbl1.pid AND t.approved = 1 AND t.to_branch = ? AND t.stock_updated_at IS NULL AND t.deleted_at IS NULL THEN td.qty END), 0) AS transfer_in, IFNULL(SUM(CASE WHEN td.product = tbl1.pid AND t.approved = 1 AND t.from_branch = ? AND t.stock_updated_at IS NULL AND t.deleted_at IS NULL THEN td.qty END), 0) AS transfer_out FROM (SELECT p.id AS pid, p.product_name AS pname, '' AS batch_number, '' AS expiry_date, 0 AS purchased_qty FROM products p WHERE IF(? > 0, p.id = ?, 1)) AS tbl1 LEFT JOIN product_transfer_details td ON tbl1.pid = td.product LEFT JOIN product_transfers t ON t.id = td.transfer_id GROUP BY tbl1.pid, td.batch_number, tbl1.expiry_date) AS tbl2 LEFT JOIN pharmacy_records pr ON tbl2.pid = pr.product LEFT JOIN pharmacies ph ON ph.id = pr.pharmacy_id GROUP BY tbl2.pid, tbl2.batch_number, tbl2.expiry_date) AS tbl3 LEFT JOIN post_operative_medicine_details pomd ON tbl3.pid = pomd.product LEFT JOIN post_operative_medicines pom ON pom.id = pomd.pom_id GROUP BY tbl3.pid, tbl3.batch_number, tbl3.expiry_date) AS tbl4 LEFT JOIN patient_medicine_records med ON tbl4.pid = med.medicine GROUP BY tbl4.pid, tbl4.batch_number, tbl4.expiry_date) AS tbl5 LEFT JOIN sales_return_details srd ON tbl5.pid = srd.product_id LEFT JOIN sales_returns sr ON sr.id = srd.return_id GROUP BY tbl5.pid, tbl5.batch_number, tbl5.expiry_date) AS tbl6 LEFT JOIN purchase_return_details prd ON prd.product_id = tbl6.pid LEFT JOIN purchase_returns pret ON pret.id = prd.return_id GROUP BY tbl6.pid, tbl6.batch_number, tbl6.expiry_date) AS tbl7 HAVING balanceQty > 0 ", [$branch, $branch, $branch, $branch, $branch, $branch, $product, $product]);
        endif;
        return $stock;
    }
