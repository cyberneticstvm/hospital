...
class HelperController extends Controller
{
    ...
    public static function getProductForTransferForEdit($product, $branch){
        if($branch == 0):
            ...
            $inventory =  DB::select("SELECT tbl1.product, tbl1.product_name, tbl1.batch_number AS batch_number, tbl1.purchased AS purchased, SUM(CASE WHEN tbl1.batch_number = t.batch_number THEN t.qty ELSE 0 END) AS transferred, tbl1.purchased-SUM(CASE WHEN tbl1.batch_number = t.batch_number THEN t.qty ELSE 0 END) AS balance_qty FROM (SELECT p.id, pr.product_name, p.product, SUM(p.qty) AS purchased, p.batch_number FROM purchase_details p LEFT JOIN products pr ON p.product=pr.id WHERE p.product = ? GROUP BY p.batch_number) AS tbl1 LEFT JOIN product_transfer_details t ON tbl1.product = t.product LEFT JOIN product_transfers pt ON pt.id=t.transfer_id WHERE pt.from_branch=0 GROUP BY tbl1.batch_number HAVING balance_qty > 0 ORDER BY tbl1.product_name", [$product]);
        else:
    ...
    public function getStockInDetailed($product, $batch, $branch){
        if($branch == 0):
            $ins = DB::table('purchase_details as pd')->leftJoin('purchases as p', 'p.id', '=', 'pd.purchase_id')->leftJoin('products as pr', 'pr.id', '=', 'pd.product')->selectRaw("SUM(pd.qty) AS qty, pd.batch_number, pr.product_name, DATE_FORMAT(p.delivery_date, '%d/%b/%Y') AS pdate")->where('pd.product', $product)->where('pd.batch_number', $batch)->orderBy('p.delivery_date')->get();
        else:
            $ins = DB::table('product_transfer_details as pd')->leftJoin('product_transfers as pt', 'pt.id', '=', 'pd.transfer_id')->leftJoin('products as pr', 'pr.id', '=', 'pd.product')->selectRaw("SUM(pd.qty) AS qty, pd.batch_number, pr.product_name, DATE_FORMAT(pt.transfer_date, '%d/%b/%Y') AS pdate")->where('pt.to_branch', $branch)->where('pd.product', $product)->where('pd.batch_number', $batch)->orderBy('pt.transfer_date')->get();
        endif;
