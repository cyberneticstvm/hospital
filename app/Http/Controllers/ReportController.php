...
class ReportController extends Controller
{
    ...
    public function fetchdaybook(Request $request){
        ...
        })->select('pref.id', 'pr.patient_id', 'pr.patient_name', 'pref.doctor_fee', 'pr.registration_fee', DB::raw("IFNULL(SUM(pp.fee), 0.00) as proc_fee"))->where('pr.branch', $request->branch)->whereBetween('pref.created_at', [$startDate, $endDate])->whereBetween('pp.created_at', [$startDate, $endDate])->where('pref.status', 1)->groupBy('pref.id')->get();
