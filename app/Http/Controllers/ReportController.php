...
class ReportController extends Controller
{
    ...
    function __construct()
    {
        ...
        $this->middleware('permission:report-hsn-product', ['only' => ['hsn', 'fetch_hsn']]);
        $this->middleware('permission:report-patient-certificate', ['only' => ['certificate', 'fetch_certificate']]);
    ...
    public function fetchdaybook(Request $request)
    {
        ...
        $pharmacy = DB::table('pharmacies as p')->leftJoin('pharmacy_records as pr', 'p.id', '=', 'pr.pharmacy_id')->where('p.branch', $request->branch)->whereNull('p.deleted_at')->whereIn('p.used_for', ['Customer'])->whereBetween('p.created_at', [$startDate, $endDate])->sum('pr.total');
    ...
    {
        $inputs = array(date('d/M/Y'), date('d/M/Y'), '', $this->branch, 'c');
        $branches = $this->getBranches($this->branch);
    ...
    public function fetchPurchase(Request $request)
    {
        ...
        if ($request->type == 'c'):
            ...
            })->selectRaw("p.id AS pid, DATE_FORMAT(p.delivery_date, '%d-%b-%Y') AS delivery_date, p.invoice_number, s.name AS sname, purchase_details.purchase_price, purchase_details.tax_amount, purchase_details.total")->get();
        endif;
    ...
    function certificate()
    {
        ...
        $inputs = array(date('Y-m-d'), date('Y-m-d'), $this->branch);
        $records = collect();
        return view('reports.certificate', compact('branches', 'records', 'inputs'));
    }
    ...
    function fetch_hsn(Request $request)
    {
        $branches = $this->getBranches($this->branch);
        $inputs = array($request->fromdate, $request->todate, $request->branch);
        $records = DB::table("patient_medicine_records as pmr")->leftJoin("products as p", "p.id", "pmr.medicine")->leftJoin("product_categories as c", "c.id", "p.category_id")->where("pmr.status", 1)->whereNull("pmr.deleted_at")->whereBetween('pmr.created_at', [Carbon::parse($request->fromdate)->startOfDay(), Carbon::parse($request->todate)->endOfDay()])->selectRaw("c.hsn, SUM(IFNULL(pmr.qty, 0)) AS qty, SUM(IFNULL(pmr.price, 0)) AS total")->when($request->branch > 0, function ($q) use ($request) {
    ...
    public function getClosingBalance($branch)
    {
        ...
        $pharmacy = DB::table('pharmacies as p')->leftJoin('pharmacy_records as pr', 'p.id', '=', 'pr.pharmacy_id')->where('p.branch', $branch)->whereNull('p.deleted_at')->whereIn('p.used_for', ['Customer'])->whereBetween('p.created_at', [$startDate, $endDate])->sum('pr.total');
