...
class ReportController extends Controller
{
    ...
    public function fetchPharmacy(Request $request)
    {
        ...
        if ($request->type == 'in'):
            ...
            $records = DB::table("pharmacies as p")->leftJoin("pharmacy_records as pr", "p.id", "pr.pharmacy_id")->whereBetween("p.created_at", [$startDate, $endDate])->where('p.branch', $request->branch)->where("p.used_for", "Customer")->when($request->product > 0, function ($q) use ($request) {
