...
class ReportController extends Controller
{
    ...
    public function fetchdaybookcc(Request $request)
    {
        ...
        $consultation_fee = DB::table('patient_medical_records as pmr')->leftJoin('patient_references as pr', 'pmr.mrn', '=', 'pr.id')->whereBetween('pr.created_at', [$startDate, $endDate])->where('pr.branch', $request->branch)->where('pr.status', 1)->sum('');
