...
class ReportController extends Controller
{
    ...
    public function fetchPharmacy(Request $request)
    {
        ...
        if ($request->type == 'in'):
            ...
            })->selectRaw("pmr.id, pmr.medical_record_id, pmr.status, p.patient_name, p.patient_id, doc.doctor_name, SUM(pmr.total) AS total, SUM(price) AS net_amount, tax_amount/2 AS cgst, tax_amount/2 AS sgst, SUM(pr.total) as invoice_amount, price AS taxable_amount, DATE_FORMAT(pmr.updated_at, '%d/%m/%Y') AS edate")->groupBy('pmr.medical_record_id')->orderByDesc('pmr.id')->get();
        else:
