...
class ReportController extends Controller
{
    ...
    public function fetchPharmacy(Request $request)
    {
        ...
        if ($request->type == 'in'):
            ...
            })->selectRaw("p.id, p.medical_record_id, '' AS status, p.patient_name, '' AS patient_id, '' AS doctor_name, SUM(pr.total) AS total, SUM(price+discount) AS net_amount, tax_amount/2 AS cgst, tax_amount/2 AS sgst, SUM(pr.total) as invoice_amount, price AS taxable_amount, DATE_FORMAT(p.updated_at, '%d/%m/%Y') AS edate")->groupBy('pr.pharmacy_id')->orderByDesc('p.id')->get();
        endif;
