...
class ReportController extends Controller
{
    ...
    public function fetchdaybookcc(Request $request)
    {
        ...
        $medicine = DB::table('patient_medical_records as p')->leftJoin('patient_medicine_records as m', 'p.id', '=', 'm.medical_record_id')->where('m.status', 1)->where('p.branch', $request->branch)->whereBetween('m.updated_at', [$startDate, $endDate])->sum('m.total');
