...
class ReportController extends Controller
{
    ...
    function fetch_hsn(Request $request)
    {
        $branches = $this->getBranches($this->branch);
        $inputs = array($request->fromdate, $request->todate, $request->branch);
        $records = DB::table("patient_medicine_records as pmr")->whereNull("pmr.deleted_at")->whereBetween('pmr.created_at', [Carbon::parse($request->fromdate)->startOfDay(), Carbon::parse($request->todate)->endOfDay()])->leftJoin("products as p", "p.id", "pmr.medicine")->leftJoin("product_categories as c", "c.id", "p.category_id")->selectRaw("c.hsn, SUM(IFNULL(pmr.qty, 0)) AS qty, SUM(IFNULL(pmr.price, 0)) AS total")->when($request->branch > 0, function ($q) use ($request) {
