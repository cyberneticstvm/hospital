...
class PDFController extends Controller
{
    ...
    public function pharmacybill($id)
    {
        ...
        $medicines = DB::table('patient_medicine_records as m')->leftJoin('products as p', 'm.medicine', '=', 'p.id')->select('p.product_name', 'p.hsn', 'm.batch_number', 'm.qty', 'm.price', 'm.tax_percentage', 'm.total', 'm.discount', 'm.tax_amount', 'm.mrp', 'pd.expiry_date', DB::raw("CASE WHEN m.eye = 'R' THEN 'RE' WHEN m.eye='L' THEN 'LE' WHEN m.eye='B' THEN 'Both Eyes' ELSE 'Oral' END AS eye"))->leftJoin('purchase_details as pd', function ($join) {
            $join->on('pd.product', '=', 'm.medicine')->on('pd.batch_number', '=', 'm.batch_number');
        })->where('m.medical_record_id', $id)->groupBy('p.product_name', 'p.hsn', 'm.batch_number', 'm.qty', 'm.price', 'm.tax_percentage', 'm.total')->get();
    ...
    public function medicalrecord($id)
    {
        ...
        $onotes = OperationNote::where('medical_record_id', $id)->first();
