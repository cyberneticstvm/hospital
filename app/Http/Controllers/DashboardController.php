...
class DashboardController extends Controller
{
    ...
    {
        $br = $this->branch;
        $tot = DB::table('patient_medicine_records')->selectRaw("SUM(total) AS total, DATE_FORMAT(updated_at, '%d/%b') AS day")->whereMonth('updated_at', Carbon::now()->month)->whereYear('updated_at', Carbon::now()->year)->where('branch_id', $br)->where('status', 1)->groupBy('day')->orderByDesc('id')->get();
