...
class DashboardController extends Controller
{
    ...
    private function getDayTotal()
    {
        ...
        $consultation_fee_total = DB::table('patient_references as pr')->whereDate('pr.created_at', Carbon::today())->where('pr.branch', $this->branch)->where('pr.status', 1)->sum(DB::raw("pr.doctor_fee-pr.discount"));

        $procedure_fee_total = DB::table('patient_procedures as pp')->where('pp.branch', $this->branch)->whereDate('pp.created_at', Carbon::today())->whereNull('pp.deleted_at')->sum('pp.fee');
        ...
        $pharmacy = DB::table('pharmacy_records as r')->leftJoin('pharmacies as p', 'p.id', '=', 'r.pharmacy_id')->whereDate('p.created_at', Carbon::today())->where("p.used_for", "Customer")->where('p.branch', $this->branch)->sum('r.total');
    ...
    public function pharmacyOutMonth()
    {
        ...
        $tot = DB::select("SELECT SUM(ph.total) AS total, DATE_FORMAT(p.updated_at, '%d/%b') AS day FROM pharmacies p LEFT JOIN pharmacy_records ph ON p.id = ph.pharmacy_id WHERE MONTH(p.updated_at) = MONTH(NOW()) AND YEAR(p.updated_at) = YEAR(NOW()) AND p.branch = ? AND p.deleted_at IS NULL AND p.stock_updated_at IS NULL AND p.used_for IN('Customer') GROUP BY day ORDER BY p.id DESC", [$br]);
        return json_encode($tot);
    }
    ...
    public function incomeExpense()
    {
        ...
        $records = DB::select("SELECT tbl1.*, IFNULL(SUM(e.amount), 0) AS expense FROM (SELECT DATE(i.created_at) AS cdate, DATE_FORMAT(i.created_at, '%d/%b') AS day, SUM(i.amount) AS income FROM patient_payments i WHERE MONTH(i.created_at) = MONTH(NOW()) AND YEAR(i.created_at) = YEAR(NOW()) AND i.branch = $br GROUP BY DATE(i.created_at) ORDER BY i.id DESC) AS tbl1 LEFT JOIN expenses e ON tbl1.cdate = e.date WHERE e.branch = $br GROUP BY tbl1.cdate ORDER BY tbl1.cdate DESC");
