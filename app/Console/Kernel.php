...
class Kernel extends ConsoleKernel
{
    ...
    private function getClosingBalance($branch)
    {
        ...
        $consultation_fee_total = DB::table('patient_medical_records as pmr')->leftJoin('patient_references as pr', 'pmr.mrn', '=', 'pr.id')->whereBetween('pr.created_at', [$startDate, $endDate])->where('pr.branch', $branch)->where('pr.status', 1)->sum('pr.doctor_fee - pr.discount');
        ...
        $income_received_other = DB::table('patient_payments')->where('branch', $branch)->whereBetween('created_at', [$startDate, $endDate])->whereIn('payment_mode', [2, 3, 4, 5, 7, 10])->where('type', '!=', 9)->where('type', '!=', 8)->sum('amount');
