...
class Kernel extends ConsoleKernel
{
    ...
    private function getClosingBalance($branch)
    {
        ...
        $pharmacy = DB::table('pharmacies as p')->leftJoin('pharmacy_records as pr', 'p.id', '=', 'pr.pharmacy_id')->where('p.branch', $branch)->whereIn('p.used_for', ['Customer'])->whereNull('p.deleted_at')->whereBetween('p.created_at', [$startDate, $endDate])->sum('pr.total');
