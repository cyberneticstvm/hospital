                ...
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('tests.advised.update', $test->id) }}" method="post" enctype="multipart/form-data">
                            ...
                            @method("PUT")
                            <div class="row g-4">
                                <div class="col-sm-3">Patient Name: <h5 class="text-primary">{{ $test->patient->patient_name }}</h5>
                                </div>
                                <div class="col-sm-3">Patient ID: <h5 class="text-primary">{{ $test->patient->patient_id }}</h5>
                                </div>
                                <div class="col-sm-3">Doctor Name: <h5 class="text-primary">{{ $test->doctor->doctor_name }}</h5>
                                </div>
                                <div class="col-sm-3">MRN: <h5 class="text-primary">{{ $test->medical_record_id }}</h5>
                                </div>
                                <div class="col-sm-6">
                                    <label class="form-label">Test Advised<sup class="text-danger">*</sup></label>
                                    <input type="text" value="{{ $tname }}" name="test_advised" class="form-control form-control-md" readonly>
                                </div>
                                <div class="col-sm-4">
