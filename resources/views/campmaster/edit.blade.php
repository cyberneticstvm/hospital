        ...
        <div class="row g-4">
            <div class="col-lg-12 col-md-12 col-sm-12">
                <div class="d-flex flex-wrap justify-content-between align-items-end">
                    <div class="mb-3">
                        <h5 class="mb-0">Edit Camp Master</h5>
                        <span class="text-muted"></span>
                    </div>
                </div>
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('campmaster.update', $camp->id) }}" method="post">
                            @csrf
                            @method("PUT")
                            <div class="row g-4">
                                <div class="col-sm-3">
                                    ...
                                    <fieldset class="form-icon-group left-icon position-relative">
                                        <input type="text" value="{{ date('d/M/Y', strtotime($camp->from)) }}" name="from" class="form-control form-control-md dtpicker" placeholder="dd/mm/yyyy">
                                ...
                                <div class="col-sm-3">
                                    ...
                                    <fieldset class="form-icon-group left-icon position-relative">
                                        <input type="text" value="{{ date('d/M/Y', strtotime($camp->to)) }}" name="to" class="form-control form-control-md dtpicker" placeholder="dd/mm/yyyy">
                                        <div class="form-icon position-absolute">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-calendar-check" viewBox="0 0 16 16">
                                                <path d="M10.854 7.146a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708 0l-1.5-1.5a.5.5 0 1 1 .708-.708L7.5 9.793l2.646-2.647a.5.5 0 0 1 .708 0z"/>
                                                <path d="M3.5 0a.5.5 0 0 1 .5.5V1h8V.5a.5.5 0 0 1 1 0V1h1a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V3a2 2 0 0 1 2-2h1V.5a.5.5 0 0 1 .5-.5zM1 4v10a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V4H1z"/>
                                ...
                                <div class="col-sm-3">
                                    ...
                                    @foreach($users as $key => $user)
                                        <option value="{{ $user->id }}" {{ $camp->cordinator == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                                    @endforeach
                                    </select>
                                    @error('cordinator')
                                ...
                                <div class="col-sm-6">
                                    ...
                                    <option value="">Select</option>
                                    @foreach($users as $key => $user)
                                        @php $selected = ''; $olds = explode(',', $camp->optometrist); @endphp
                                        @foreach($olds as $key1 => $value)
                                            @if($user->id == $value)
                                                {{ $selected = 'selected' }}
                                            @endif
                                        @endforeach
                                ...
                                <div class="col-sm-3">
                                    ...
                                    @foreach($ctypes as $key => $type)
                                        <option value="{{ $type->id }}" {{ $camp->type == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                                    @endforeach
                                    </select>
                                    @error('type')
