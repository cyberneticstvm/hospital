...
<body>
    ...
    <table width="100%">
        <thead>
            <tr>
                <th text-align="center" colspan="4">B-SCAN BILL</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                ...
                <td>Bill Number</td>
                <td>{{ $bscan->receipt_number }}</td>
            </tr>
            <tr>
                <td>Patient Name / ID / MR.ID</td>
                <td>{{ $patient->patient_name }} / {{ $patient->patient_id }} / {{ $bscan->medical_record_id }}</td>
                <td>Date</td>
                <td>{{ date('d/M/Y', strtotime($bscan->created_at)) }}</td>
            </tr>
            <tr>
                <td>Advised By</td>
