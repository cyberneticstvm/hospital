...
<body>
    ...
    <tr>
        <th width="10%">SL No</th><th>Date</th><th width="50%">Test</th><th>Qty</th><th>Amount</th><th>Total</th>
    </tr>
    ...
    @endphp
    <tr>
        <td>{{ $c++ }}</td><td>{{ date('d/M/Y', strtotime($lab->result_updated_on)) }}</td><td>{{ $test->lab_type_name }}</td><td class="text-right">1</td><td class="text-right">{{ $test->fee }}</td><td class="text-right">{{ $test->fee }}</td>
    </tr>
    @php $tot += $test->fee; @endphp
    @endforeach
    <tr><td colspan="5" class="text-right">Total</td><td class="text-right">{{ number_format($tot, 2) }}</td></tr>
    </table>
