...
<body>
    ...
    <br/>
    <table width="100%">
        <thead><tr><th text-align="center" colspan="4">HFA BILL</th></tr></thead>
        <tbody>
            <tr>
                <td>Patient Name / ID / MR.ID</td>
                <td>{{ $patient->patient_name }} / {{ $patient->patient_id }} / {{ $hfa->medical_record_id }}</td>
                <td>Date</td><td>{{ date('d/M/Y', strtotime($hfa->created_at)) }}</td>
            </tr>
        </tbody>
    </table>
    <br />
