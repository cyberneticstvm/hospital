...
<body>
    ...
    <br>
    <table width="100%">
        <thead><tr><th text-align="center" colspan="4">VISION EXAMINATION BILL</th></tr></thead>
        <tbody>
            <tr>
                <td>Patient Name / ID / MR.ID</td>
                <td>{{ $patient->patient_name }} / {{ $patient->patient_id }} / {{ $spectacle->medical_record_id }}</td>
                <td>Date</td><td>{{ date('d/M/Y', strtotime($spectacle->created_at)) }}</td>
            </tr>
        </tbody>
    </table>
    <br>
    <table class="bordered" width="100%" cellspacing="0" cellpadding="0">
        ...
        <tfoot><tr><td colspan="3" class="text-right"><strong>Total</strong></td><td class="text-right"><strong>{{ number_format($spectacle->fee, 2) }}</strong></td></tr></tfoot>
    </table>
</body>
</html>