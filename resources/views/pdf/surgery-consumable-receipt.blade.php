...
<body>
    ...
    <table width="100%" cellpadding='0' cellspacing='0' class="text-large bordered">
        <tr>
            <td>MR ID</td>
            <td>{{ $psc->medical_record_id }}</td>
            <td>Patient ID</td>
            <td>{{ $psc->patient->patient_id }}</td>
            <td>Name/Age/Gender</td>
            <td>{{ $psc->patient->patient_name }} / {{ $psc->patient->age }} / {{ $psc->patient->gender }}</td>
        </tr>
        <tr>
            <td>Address</td>
    ...
    <table width="100%" border="1" cellpadding='1' cellspacing='0' class="text-large">
        <tr>
            ...
            <th width="70%">Consumable Name</th>
        ...
        @endforeach
        <tr>
            <td colspan="4" class="text-right fw-bold"></td>
            <td class="text-right fw-bold">{{ $psc->psclist()->sum('total') }}</td>
        </tr>
        <tr>
            <td colspan="4" class="text-right fw-bold">Discount / Insurance / Others</td>
            <td class="text-right fw-bold">{{ $psc->discount }}</td>
        </tr>
        <tr>
            <td colspan="4" class="text-right fw-bold">Total</td>
            <td class="text-right fw-bold">{{ $psc->total_after_discount }}</td>
        </tr>
    </table>
