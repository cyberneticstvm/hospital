    ...
    <div class="container">        
        <div class="row g-4">
            <div class="col-lg-12 col-md-12 col-sm-12">
                ...
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('reports.fetchincomeexpense') }}" method="post">
                            @csrf
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-sm-3">
                                    ...
                                    <option value="I" {{ ($inputs && $inputs[3] == 'I') ? 'selected'  : '' }}>Income</option>
                                    <option value="E" {{ ($inputs && $inputs[3] == 'E') ? 'selected'  : '' }}>Expense</option>
                                    </select>
                                    @error('type')
                                ...
                                <div class="col-sm-3">
                                    ...
                                    <select class="form-control form-control-md select2" data-placeholder="Select" name="head">
                                    <option value="">Select</option>
                                    @foreach($heads as $key => $head)
                                        <option value="{{ $head->id }}" {{ ($inputs && $inputs[4] == $head->id) ? 'selected'  : '' }}>{{ $head->name }}</option>
                                    @endforeach
                                    </select>
                                    @error('head')
                ...
                <div class="card">
                    <div class="card-body">
                        <table class="table table-sm dataTable table-striped table-hover align-middle">
                            <thead><tr><th>SL No.</th><th>Date</th><th>Branch</th><th>Head</th><th>Type</th><th>Description</th><th class="text-end">Amount</th></tr></thead><tbody>
                            ...
                            @empty
                            @endforelse
                            <tr><td colspan="6" class="text-end fw-bold">Total</td><td class="text-end fw-bold">{{ number_format($tot, 2) }}</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
